...
def format_spot_info(spot: dict, distance_m: float | None = None,
    ...
    if include_live:
        if not get_cwa_key():
            lines.append("⚠️ 即時潮汐/風況數據（CWA API Key 未設定）")
        else:
            # 潮汐
    ...
    else: # 離線模式或不查詢 CWA 時的提示
        lines.append("⚠️ 即時潮汐/風況/颱風數據：離線無法獲取")
...
def main():
    ...
    parser.add_argument("--offline-cwa", action="store_true",
                        help="在離線模式下不查詢 CWA 即時資料")
    parser.add_argument("--list", action="store_true", help="列出所有浪點")
    args = parser.parse_args()

    spots = load_spots()
    include_parking = args.mode in ("parking", "all")
    include_live = not args.offline_cwa # 如果有 --offline-cwa，則 include_live 為 False

    if not get_cwa_key() and include_live: # 只有在線模式才提示 CWA_API_KEY
